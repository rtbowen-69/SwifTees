...
describe('SwifTees', () => {
  ...
  describe('Minting', () => {
    ...
    describe('Success', async () => {
      ...
      it('returns address of minter', async () => {
        expect(await swiftees.ownerOf(1)).to.equal(minter.address)              
      })
    ...
    describe('Failure', async () => {
      ...
      it('does not allow more than 4 Tickets per wallet', async () => {
        await expect(swiftees.connect(minter).mint(5, { value: COST })).to.be.reverted
      })
      ...
      it('rejects presale minting before presale is open', async () => {        
        await expect(swiftees.connect(minter).mint(4, { value: COST })).to.be.reverted
      })
  ...
  describe('Displaying NFTs', () => {
    ...
    beforeEach(async () => {
      ...
      transaction = await swifTees.connect(minter).mint(3, { value: COST * 3 })
      result = await transaction.wait()
    })
